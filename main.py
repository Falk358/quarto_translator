...
def makeEstimate(loader: DocLoader):
    ...
    counter.printCostEstimateTranslation(
        pricing_input=0.0015, pricing_output=0.002
    )  # pricing for gpt-3.5 Turbo


def main():
    arg_parse = argparse.ArgumentParser(
        prog="quartoTranslator",
        description="translate written text in Quarto file from source language to target language, leaving syntax mostly intact.",
        epilog="Usage: python3 main.py -c template.config",
    )
    arg_parse.add_argument("-c", "--config", help="path to specified config file")
    arg_parse.add_argument(
        "-s", "--select", action="store_true", help="select which files to process"
    )
    ...
    translator = Translator(
        file_chunk_dict=text_splits, parser=configparser, model_name=model
    )
    if cmd_args.select:
        ...
        selected_indices = input(
            "Enter the numbers of files to process (comma-separated): "
        ).split(",")
